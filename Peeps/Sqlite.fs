...
type SqliteContext =
    ...
    static member Create(path, name) =
        ...
        let dbName = sprintf "%s_%s.log" name (reference.ToString())    
    ...
    member ctx.CreateConnection() =
        let conn = new SqliteConnection(sprintf "Data Source=%s" ctx.Path)
        conn.Open()
        conn
          ...
          ParameterMapper =
              Some
                  ((fun (item: PeepsLogItem) ->
                      ...
                      Map.ofList [
                          "@item_type", itemType :> obj
                          "@date_time", item.TimeUtc.ToString() :> obj
                          "@timestamp", DateTimeOffset(item.TimeUtc).ToUnixTimeMilliseconds() :> obj
                          "@name", item.From :> obj
...
type DbWriter(path, name) =
    ...
    let handleItem connection item =
        let _ = Internal.addLogItemQuery.Execute(connection, item)        
        true
