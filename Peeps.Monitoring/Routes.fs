...
module PeepsMetricRoutes =
    ...
    let requestMetrics: HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            let logger =
                ctx.GetLogger("request-metrics")

            logger.LogInformation "Hello, from metrics"

            let service =
                ctx.GetService<PeepsMonitorAgent>()
    ...
    let logMetrics: HttpHandler =
        fun (next: HttpFunc) (ctx: HttpContext) ->
            ...
            let itemCount =
                service.ItemCount() |> string

            text itemCount next ctx

    let routes: (HttpFunc -> HttpContext -> HttpFuncResult) list =
        [ GET
          >=> choose [ route "/metrics/requests/"
                       >=> warbler (fun _ -> requestMetrics)
                       route "/metrics/log/"
                       >=> warbler (fun _ -> logMetrics) ] ]
